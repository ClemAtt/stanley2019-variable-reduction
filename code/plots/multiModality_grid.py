...
class AEP_calc(Component):
    ...
    def solve_nonlinear(self, params, unknowns, resids):
        ...
        AEP, daep_dx, daep_dy = fast_calc_gradients(
            turbineX,
            turbineY,
            turbineZ,
            rotorDiameter,
...
class constraint_calc(Component):
    ...
    def solve_nonlinear(self, params, unknowns, resids):
        ...
        unknowns["spacing_constraint"] = ss - (2.0 * rotorDiameter[0]) ** 2
...
class form_grid(Component):
    ...
    def __init__(self, nTurbines, nRows):
        ...
        # Explicitly size input arrays
        self.add_input("dx", val=0.0)
        self.add_input("dy", val=0.0)
        self.add_input("shear", val=0.0)
        self.add_input("rotate", val=0.0)
        self.add_input("y0", val=0.0)
    ...
    def solve_nonlinear(self, params, unknowns, resids):
        ...
        sheard = 0.0
        rotated = 0.0
        turbineX, dx_ddx, turbineY, dy_ddx = grid_param_fortran.makegrid_fortran_dv(
            nTurbines,
            dx,
            dxd,
            dy,
        ...
        sheard = 0.0
        rotated = 0.0
        turbineX, dx_ddy, turbineY, dy_ddy = grid_param_fortran.makegrid_fortran_dv(
            nTurbines,
            dx,
            dxd,
            dy,
        ...
        (
            ...
            turbineY,
            dy_dshear,
        ) = grid_param_fortran.makegrid_fortran_dv(
            nTurbines,
            dx,
            dxd,
            dy,
...
if __name__ == "__main__":
    ...
    if boundary == "circle":
        ...
        locations = np.loadtxt(
            "/Users/ningrsrch/Dropbox/Projects/reduction/layout_amalia.txt"
        )
        xBounds = locations[:, 0]
        yBounds = locations[:, 1]
    ...
    num = 200
    d_dx = np.linspace(2.0 * rotorDiameter[0], 4.0 * rotorDiameter[0], num)
    # AEP_dx = np.zeros(num)
    # for i in range(num):
    #         print i
    #         prob['dx'] = d[i]
    ...
    d_dy = np.linspace(2.0 * rotorDiameter[0], 4.0 * rotorDiameter[0], num)
    # AEP_dy = np.zeros(num)
    # for i in range(num):
    #         print i
    #         prob['dx'] = dx_start
    ...
    d_shear = np.linspace(-2.0 * rotorDiameter[0], 2.0 * rotorDiameter[0], num)
    ...
    d_rot = np.linspace(-30.0, 30.0, num)
    ...
    ax1.set_xticks(
        [2.0 * rotorDiameter[0], 3.0 * rotorDiameter[0], 4.0 * rotorDiameter[0]]
    )
    ...
    ax3.set_xticks([-2.0 * rotorDiameter[0], 0.0, 2.0 * rotorDiameter[0]])
    ax3.set_xticklabels(["-2", "0", "2"])
    ax3.set_xlabel(r"$b$ " + "(rotor\ndiameters)")

    ax4.plot(d_rot, AEP_rot, color="C2", alpha=0.5)
    ax4.set_xticks([-30.0, 0.0, 30.0])
    ax4.set_xticklabels(["-30", "0", "30"])
    ax4.set_xlabel(r"$\theta$ (degrees)")

    ax1.set_xlim(2.0 * rotorDiameter[0], 4.0 * rotorDiameter[0])
    ax2.set_xlim(2.0 * rotorDiameter[0], 4.0 * rotorDiameter[0])
    ax3.set_xlim(-2.0 * rotorDiameter[0], 2.0 * rotorDiameter[0])
    ax4.set_xlim(-30.0, 30.0)

    ax1.set_yticks([725000.0, 775000.0, 825000.0, 875000.0])
    ax2.set_yticks([725000.0, 775000.0, 825000.0, 875000.0])
    ax3.set_yticks([725000.0, 775000.0, 825000.0, 875000.0])
    ax4.set_yticks([725000.0, 775000.0, 825000.0, 875000.0])
    ...
    ax1.set_ylim(725000.0, 875000.0)
    ax2.set_ylim(725000.0, 875000.0)
    ax3.set_ylim(725000.0, 875000.0)
    ax4.set_ylim(725000.0, 875000.0)
    ...
    # ax4.set_yticklabels([''])
    plt.subplots_adjust(
        top=0.93, bottom=0.3, right=0.96, left=0.1, hspace=0.0, wspace=0.3
    )
