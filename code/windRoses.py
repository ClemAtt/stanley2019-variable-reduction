...
def novatoRose(nDirections, nSpeeds=1):
    data = np.array(
        [
            [0.839, 0.710, 0.562, 0.591, 0.041, 0.003],
    ...
    for i in range(m):
        for j in range(n):
            ws[i] += data[i][j] / 100.0 * speeds[j]
        ws[i] = ws[i] / wf[i]

    wd = np.linspace(0.0, 360.0 - 360 / float(m), m)
...
def ukiahRose(nDirections, nSpeeds=1):
    data = np.array(
        [
            [0.479, 0.639, 0.516, 0.471, 0.025, 0.002],
...
def victorvilleRose(nDirections, nSpeeds=1):
    data = np.array(
        [
            [0.484, 0.289, 0.176, 0.136, 0.017, 0.015],
...
def newhallRose(nDirections, nSpeeds=1):
    data = np.array(
        [
            [8.192, 2.952, 1.761, 1.511, 0.367, 0.108],
...
def bealeRose(nDirections, nSpeeds=1):
    data = np.array(
        [
            [0.856, 0.347, 0.149, 0.099, 0.027, 0.009],
...
def northIslandRose(nDirections, nSpeeds=1):
    data = np.array(
        [
            [1.194, 0.560, 0.216, 0.090, 0.009, 0.003],
...
def alturasRose(nDirections, nSpeeds=1):
    data = np.array(
        [
            [0.447, 0.281, 0.233, 0.292, 0.054, 0.015],
...
def amaliaRose(nDirections, nSpeeds=1):
    ...
    windDirections = np.linspace(
        0.0, 360.0 - 360.0 / float(len(windSpeeds)), len(windSpeeds)
    )
    dirs, frequencies, speeds = process_rose_data(
        windSpeeds, windDirections, windFrequencies, nDirections, nSpeeds
    )
...
def setup_weibull(windDirections, windFrequencies, windSpeeds, nSpeeds):
    ...
    for i in range(nDirections):
        ...
        speed_dist = np.linspace(
            (25.0) / (2.0 * float(nSpeeds)) + 0.001,
            25.0 - (25.0) / (2.0 * float(nSpeeds)),
            nSpeeds,
        )
        ...
        num_int = 1000
        for j in range(nSpeeds):
            speed_int = np.linspace(
                speed_dist[j] - dspeed / 2.0, speed_dist[j] + dspeed / 2.0, num_int
            )

            k = 2.0
            scale = avg_speed / (spec.gamma(1.0 + 1.0 / k))
...
def process_rose_data(
    ...
    num = nDirections
    dirs = np.linspace(0.0, 360.0 - 360.0 / float(num), num)
    ddir = dirs[1] - dirs[0]
    ...
    dir_int1 = np.linspace(dirs[0], dirs[0] + ddir / 2.0, num_int / 2)
    ...
    dir_int2 = np.linspace(dirs[0], dirs[0] + ddir / 2.0, num_int / 2)
    ...
    speeds[0] = (
        np.trapz(speed_freq_int1, dir_int1) + np.trapz(speed_freq_int2, dir_int2)
    ) / (np.trapz(freq_int1, dir_int1) + np.trapz(freq_int2, dir_int2))

    for i in range(1, num):
        dir_int = np.linspace(dirs[i] - ddir / 2.0, dirs[i] + ddir / 2.0, num_int)
...
if __name__ == "__main__":
    ...
    wd -= wd[np.argmax(wf)]
    wd += 270.0
    wd += 180.0 / float(nDirections)
    for i in range(nDirections):
        wd[i] = radians(wd[i]) * -1.0
    """wind rose"""
    plt.figure(1)
    max_height = max(wf)
    ...
    ax.set_xticklabels(
        ["E", "NE", "N", "NW", "W", "SW", "S", "SE"], fontsize=18, family="serif"
    )
