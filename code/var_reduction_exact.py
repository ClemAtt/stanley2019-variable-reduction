...
def startLoop(
    ...
    """Starts a loop to fill an arbitrary boundary with a baseline grid
    Parameters:
    -----------
    N: int
        Number of turbines
    d: float
        Turbine diameter
    my: float
        Turbine spacing factor
    step: float
        Step size for grid expansion
    plus_minus: int
        1 or -1 for grid expansion
    rotate: float
        Rotation of grid
    turbs_per_row: list
    ...
    y0: float
        Starting y position
    boundaryVertices: list
        Boundary vertices
    boundaryNormals: list
        Boundary normals
    t: int
        Number of rows
    Returns:
    --------
    num_inside: int
        Number of turbines inside boundary
    """

    d = d + plus_minus * d * step

    shear = np.tan(np.deg2rad(20.0)) * d
    X, Y = full_param.makegrid_fortran(
        t, N, d, d * my, shear, rotate, turbs_per_row, x_start, y0
    )
    ...
    if plot:
        ...
        for i in range(len(X)):
            circ = plt.Circle((X[i], Y[i]), 130.0 / 2.0)
...
def startGrid(
    nTurbines, boundaryVertices, boundaryNormals, rotate=0.0, my=1.0, step=0.25, t=25
):
    """Starts a grid to fill an arbitrary boundary
    ...
    --------
    X: list
        X positions of turbines
    Y: list
        Y positions of turbines
    ...
    shear = np.tan(np.deg2rad(20.0)) * d
    X, Y = full_param.makegrid_fortran(
        nRows, N, d, d * my, shear, rotate, turbs_per_row, x_start, y0
    )
    ...
    for i in range(nTurbines - 1):
        if (
            turbineY[i + 1] < turbineY[i] + 1e-4
            and turbineY[i + 1] > turbineY[i] - 1e-4
        ):
            turbs_per_row[-1] += 1
        else:
            turbs_per_row = np.append(turbs_per_row, 1)

    shear = dy * np.tan(np.deg2rad(20.0))
    ...
    for i in range(nRows - 1):
        x_start[i + 1] = (
            x_start[i]
            + (
                turbineX[sum(turbs_per_row[0 : i + 1])]
                - shear
                - turbineX[sum(turbs_per_row[0:i])]
            )
            / dx
        )
    ...
    for i in range(nRows):
        for j in range(turbs_per_row[i]):
            ...
            y[index] = (
                y0
                + dy * i
                + (float(j) + col_start[i]) * dx * np.tan(np.deg2rad(shearY))
            )
            index += 1
    xc = x - (max(x) + min(x)) / 2.0
    yc = y - (max(y) + min(y)) / 2.0
...
def startTurbines(xBounds, yBounds, start, diameter, nTurbs, frac=0.5, my=2.0):
    ...
    dx, dy, shearX, shearY, rotate, turbs_per_row, x_start, y0, col_start = startGrid(
        turbsRemaining, boundaryVertices, boundaryNormals, my=my
    )
...
def turbineLocs(
    ...
    boundX, boundY = makeBoundary(xBounds, yBounds, start, diameter, nTurbs, frac=frac)
    gridX, gridY = makeGrid(
        dx, dy, shearX, shearY, rotate, turbs_per_row, x_start, y0, col_start
    )
...
if __name__ == "__main__":
    ...
    (
        ...
        y0,
        col_start,
    ) = startTurbines(xBounds, yBounds, start, diameter, nTurbs, frac=0.5, my=1.0)
    turbineX, turbineY = turbineLocs(
        xBounds,
        yBounds,
        start,
        diameter,
